        ...
        ActivationMode::PushToTalk => match event_type {
            ...
            KeyEventType::Released => {
                if *recording_source == target {
                    // Symmetric with ToggleToTalk: drop Release events that arrive
                    // within the cooldown of the last start so synthetic Release+Press
                    // pairs (X11 auto-repeat or Wayland portal rafales) can't
                    // stop the recording mid-utterance.
...
mod tests {
    ...
    fn cooldown_threshold_matches_documented_value() {
        // PTT/Toggle behaviour assumes 250 ms — making this constant accidentally
        // larger would let auto-repeat noise through; smaller would drop legit
        // very-short taps.
