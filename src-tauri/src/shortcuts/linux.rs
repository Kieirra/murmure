    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            // Toggle mode: separate start and stop shortcuts
                ...
                let all_start_keys_down =
                    !start_keys.is_empty() && start_keys.iter().all(|k| pressed.contains(k));
                let all_stop_keys_down =
                    !stop_keys.is_empty() && stop_keys.iter().all(|k| pressed.contains(k));
                ...
                if !last_transcript_pressed && !last_transcript_required_keys.is_empty() {
                    let all_last_transcript_keys_down = last_transcript_required_keys
                        .iter()
                        .all(|k| pressed.contains(k));
