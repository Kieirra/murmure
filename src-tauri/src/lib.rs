...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler({
            // The invoke_handler set is duplicated under cfg so the four
            // `__test_*` commands only exist when the `audio-injection`
            // feature is enabled. Keeping a single source of truth via macro
            // would require building a function-token macro, which obscures
            // the call site more than this explicit duplication does.
            #[cfg(feature = "audio-injection")]
            {
                tauri::generate_handler![
                    ...
                    __test_set_audio_source,
                    __test_press_record_shortcut,
                    __test_release_record_shortcut,
