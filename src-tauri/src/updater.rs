...
/// Initialize the updater with the appropriate Linux format target
///
/// Note: The updater plugin's Builder doesn't expose a target() method in v2,
/// so the custom target is passed at runtime via the check() function instead.
#[allow(dead_code)]
pub fn init_updater_with_format() -> tauri_plugin_updater::Builder {
