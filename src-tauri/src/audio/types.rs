...
pub struct AudioState {
    ...
    pub streaming_handle: std::sync::Mutex<Option<std::thread::JoinHandle<()>>>,
    pub streaming_stop: Arc<AtomicBool>,
    pub streaming_buffer: Arc<std::sync::Mutex<Vec<f32>>>,
}
...
impl AudioState {
    pub fn new() -> Self {
        Self {
            ...
            streaming_handle: std::sync::Mutex::new(None),
            streaming_stop: Arc::new(AtomicBool::new(false)),
            streaming_buffer: Arc::new(std::sync::Mutex::new(Vec::new())),
        }
    }
